...
server <- function(input, output,session) {
  ...
  output$barPlotFiltered <- renderPlot({
    data_filtered <- data_bar %>%
      filter(Category %in% c("Same Money but Less Equity", "More Money but Same Equity"))
    ...
    ggplot(data_filtered, aes(x=reorder(Category, -Count), y=Count, fill=Category)) +
      ...
      labs(title="Focused Analysis on Specific Deal Types",
           x="Categories", y="Number of Instances") +
      scale_fill_manual(values = c("Same Money but Less Equity" = "green", "More Money but Same Equity" = "green")) +
      theme(
  ...
  output$treeMapPlot2 <- renderPlot({
    if(input$showTreeMap2) {  # Conditional rendering based on the new checkbox for the second treemap
      ...
      # Extract the industry information for the filtered lists
      investment_columns <- c("Startup.Name", 
  ...
  plot_deal_closure_rate <- function(shark_name, data) {
    ...
    percentage_total_pitches <- ifelse(total_pitches_present > 0, 100, NA)
    percentage_deals_closed <- ifelse(total_pitches_present > 0, round(deals_closed / total_pitches_present * 100), NA)
    
    plot_data <- data.frame(
      Category = c("Total Pitches", "Deals Closed"),
      Count = c(total_pitches_present, deals_closed),
      Percentage = c(percentage_total_pitches, percentage_deals_closed)
    )
  ...
  # Output for the first selected shark
  output$dealPlot <- renderPlot({
    plot_deal_closure_rate(input$sharkInput, shark_tank_data)
  })
